...
PLAY2VEC_PATH = "models/gp1/multi_token/play2vec.npy"
PLAY_INDEX_PATH = "models/gp1/multi_token/play2vec_index.json"
EVENTS_DIR = "data"
OUT_DIR = "models/gp1/multi_token"

print("Loading Play2Vec...")
play_vectors = np.load(PLAY2VEC_PATH)

# play2vec_index.json is a LIST
with open(PLAY_INDEX_PATH, "r") as f:
    play_index_list = json.load(f)

# Build: (match_id, period, possession) -> row index
play_index = {}
...
np.save(f"{OUT_DIR}/player_match2vec.npy", player_match_vectors)

with open(f"{OUT_DIR}/player_match2vec_index.json", "w") as f:
    json.dump(player_match_index, f)

print("PlayerMatch2Vec saved")
