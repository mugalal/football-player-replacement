...
# ==================================================================================================
# TOKEN-BUDGET POOLING — equalized contribution per role
# ==================================================================================================

ROLE_TOKEN_BUDGETS = {
    # Each role contributes the same total tokens to the pooled query,
    # regardless of how many matches the player has or how event-dense they were.
    # This prevents Coutinho (high event volume) from dominating the multi-source query.
    "onball_per_role":  400,
    "offball_per_role": 100,
}


def gather_multi_source_docs_equalized(
    sources_config,
    metadata: dict,
    player_ids: list,
    onball_budget: int = 400,
    offball_budget: int = 100,
):
    """
    Pool documents from multiple sources but cap total tokens per role.

    For each source player:
        - Use ALL their available match docs
        - Concatenate all their onball tokens, shuffle, truncate to onball_budget
        - Same for offball
    This produces ONE merged "role document" per source player, all contributing
    equally to the final pooled query.

    Returns (pooled_role_docs, summary) where each role_doc is a synthetic
    document representing one role's equalized contribution.
    """
    pooled_role_docs = []
    summary = []

    for name, _target_count_unused, role_desc in sources_config:
        pid = find_player_id(name, metadata, player_ids)
        if pid is None:
            summary.append((name, role_desc, 0, 0, "not found"))
            continue

        meta = metadata[pid]
        if "Liverpool" not in meta.get("team", ""):
            summary.append((name, role_desc, 0, 0, f"primary team is {meta.get('team', 'Unknown')}"))
            continue

        docs = load_player_docs(pid)
        if not docs:
            summary.append((name, role_desc, 0, 0, "no match docs"))
            continue

        # Concatenate ALL onball and offball tokens from ALL matches
        all_onball = []
        all_offball = []
        for doc in docs:
            all_onball.extend(doc.get("onball_tokens", []) or [])
            all_offball.extend(doc.get("offball_tokens", []) or [])

        # Shuffle so the truncation isn't biased toward early-match tokens
        random.shuffle(all_onball)
        random.shuffle(all_offball)

        # Truncate to the role budget — equalized contribution
        truncated_onball = all_onball[:onball_budget]
        truncated_offball = all_offball[:offball_budget]

        # Build one synthetic "role document"
        role_doc = {
            "player_id": pid,
            "match_id": f"{pid}_pooled",
            "primary_position": meta.get("primary_position", "Unknown"),
            "onball_tokens": truncated_onball,
            "offball_tokens": truncated_offball,
        }
        pooled_role_docs.append(role_doc)

        summary.append((
            meta["name"],
            role_desc,
            len(truncated_onball),
            len(truncated_offball),
            f"OK ({len(docs)} matches, {len(all_onball)} total onball)"
        ))

    return pooled_role_docs, summary


def validate_mane_multi_source_equalized():
    """
    Approach B-Equalized: Same multi-source pooling as Approach B, but every role
    contributes the same total tokens. This neutralizes Coutinho's volume bias.
    """
    print("\n" + "█" * 105)
    print("APPROACH B-EQUALIZED — Multi-Source with Equalized Token Contribution Per Role")
    print("Each Liverpool attacker contributes the same number of tokens, regardless of match count")
    print("█" * 105)

    player_ids, vectors, metadata, onball_model, offball_model = load_everything()

    # ----- 1. Gather equalized role docs -----
    print("\nGathering equalized role documents:")
    role_docs, summary = gather_multi_source_docs_equalized(
        LIVERPOOL_MULTI_ROLE_SOURCES,
        metadata,
        player_ids,
        onball_budget=ROLE_TOKEN_BUDGETS["onball_per_role"],
        offball_budget=ROLE_TOKEN_BUDGETS["offball_per_role"],
    )

    print(f"\n{'Player':<35}{'Role':<28}{'Onball':<10}{'Offball':<10}{'Status':<40}")
    print("-" * 125)
    for name, role, ob, off, status in summary:
        print(f"{name:<35}{role:<28}{ob:<10}{off:<10}{status:<40}")

    if len(role_docs) < 3:
        print("⚠ Too few role docs — aborting equalized test")
        return

    print(f"\nTotal role docs: {len(role_docs)}")
    print(f"Each role contributes ≤ {ROLE_TOKEN_BUDGETS['onball_per_role']} onball + "
          f"≤ {ROLE_TOKEN_BUDGETS['offball_per_role']} offball tokens")

    # ----- 2. Apply Klopp interventions -----
    print("\nApplying Klopp-style interventions to equalized role documents...")
    ...
    for doc in role_docs:
        ...
        modified_role_docs.append(new_doc)

    # ----- 3. Re-infer target vector -----
    print("Inferring equalized multi-source target vector...")
    target_vec = infer_player_vector_from_docs(modified_role_docs, onball_model, offball_model)

    # ----- 4. Search -----
    source_pids = []
    for name, _, _, _, status in summary:
    ...
    print("\nTop-30 most similar players to 'EQUALIZED multi-zone Klopp ideal attacker':")
    ...
    if mane_pid:
        ...
        print(f"  Mané's versatility: {mane_versatility:.3f}")

    # ----- 7. Verdict -----
    print("\n" + "█" * 105)
    print("APPROACH B-EQUALIZED VERDICT")
    print("█" * 105)
    if target_rank is None:
        print("  Mané not in top-30. See diagnostic above for actual rank.")
    elif target_rank <= 5:
        print(f"  ✓✓✓ EXCELLENT — Mané ranked #{target_rank}")
    elif target_rank <= 10:
        print(f"  ✓✓ STRONG — Mané ranked #{target_rank}")
    elif target_rank <= 20:
        print(f"  ✓ GOOD — Mané ranked #{target_rank}")
    elif target_rank <= 30:
        print(f"  ⚠ MARGINAL — Mané ranked #{target_rank}")
    else:
        print(f"  ⚠ Mané ranked #{target_rank}")
...
def main():
    ...
    validate_mane_multi_source_equalized()
